...
const ERROR_MESSAGES: Record<string, string> = {
  account_not_found:
    "Akun Google ini belum terdaftar. Silakan daftar terlebih dahulu melalui halaman Register.",
  google_auth_failed: "Login dengan Google gagal. Coba lagi.",
  session_exchange_failed: "Gagal memverifikasi sesi Google. Coba lagi.",
  missing_code: "Terjadi kesalahan autentikasi. Coba lagi.",
};
...
export function LoginPageContent() {
  ...
  const displayError =
    error || (urlErrorCode ? ERROR_MESSAGES[urlErrorCode] : "");
  ...
  async function handleGoogleLogin() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/api/auth/callback?mode=login`,
        queryParams: {
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                {displayError ? (
                  <div className="rounded-lg border border-red-100 bg-red-50 px-3 py-2">
                    ...
                    {urlErrorCode === "account_not_found" && !error && (
                      <Link
                        href="/register"
                        className="mt-1 block text-sm font-bold text-red-800 underline hover:text-red-900"
                      >
                        Daftar sekarang →
                      </Link>
                    )}
