...
export function HeroDashboardPreview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const fGrams = Math.round((target * 0.30) / 9);
  ...
  const macros = [
    { label: "Protein", value: data.protein, unit: "g", pct: 25, color: "#16a34a" },
  ...
  return (
    <div className="animate-float rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
      ...
      {/* Body: Today's Plan + Nutrition Summary */}
      <div className="grid grid-cols-2 gap-3">
        {/* Today's plan */}
        <div>
          <p className="mb-2 text-xs font-bold text-ink">Today&apos;s Plan</p>
          {data.todayMeals.map((meal, i) => (
            <div key={i} className="mb-2 flex items-center gap-2 rounded-lg bg-slate-50 p-2">
              ...
              <div className="min-w-0">
                <p className="truncate text-[11px] font-semibold text-ink">{meal.name}</p>
        ...
        <div>
            ...
            <div className="space-y-1.5">
              {macros.map((m) => (
                <div key={m.label} className="flex items-center gap-1.5">
                  ...
                  <span className="ml-auto text-[10px] font-bold text-ink">{m.pct}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.allergies.slice(0, 4).map((a) => {
              ...
              return (
                <span key={a} className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2 py-0.5 text-[10px] font-semibold text-red-700 ring-1 ring-red-100">
